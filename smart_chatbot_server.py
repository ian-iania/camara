...
def generate_response(query, documents, chat_history=None):
    ...
    messages = [
        {"role": "system", "content": f"Você é um assistente útil que responde perguntas com base nos documentos fornecidos. Use apenas as informações dos documentos para responder. Se a informação não estiver nos documentos, diga que não tem essa informação. Quando listar itens numerados ou com marcadores, coloque cada item em uma linha separada. Quando apresentar parágrafos, separe-os com linhas em branco. Contexto dos documentos:\n\n{context}"},
    ]
        ...
        function addMessage(content, isUser) {
            ...
            if (isUser) {
                ...
                const formattedContent = content
                    ...
                    .replace(/\n(\d+\.\s)/g, '</p><p>$1') // Numbered list items on new lines
                    .replace(/\n(\*\s|\-\s)/g, '</p><p>$1') // Bullet list items on new lines
